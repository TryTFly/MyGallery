...
public class MyIDCheck extends Activity{
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        temp();  
        Toast.makeText(MyIDCheck.this, "注意：相册的初始密码为admin!", Toast.LENGTH_SHORT).show();
    }
	...
	public void alter(){//修改页面
		LayoutInflater factory = LayoutInflater.from(MyIDCheck.this);//得到自定义对话框
    ...
    class DialogInterfaceOnClick2 implements DialogInterface.OnClickListener{//登陆
		...
		public void onClick(DialogInterface dialog, int whichButton) {
			...
			if(pass.equals(password01)){
				Toast.makeText(MyIDCheck.this, "登录成功", Toast.LENGTH_SHORT).show();
				...
				Toast.makeText(MyIDCheck.this, "密码错误！", Toast.LENGTH_SHORT).show();
				temp();
			}
		}
    }
    ...
    class DialogInterfaceOnClick6 implements DialogInterface.OnClickListener{//退出
		...
		public void onClick(DialogInterface dialog, int whichButton) {
			MyIDCheck.this.finish();//点击退出按钮之后调用finish方法结束应用程序			
		}
...
//				editor = shared.edit();  //打开指定shared文件，如果不存在则创建他
				...
				editor.commit();//提交	
				temp();
				Toast.makeText(MyIDCheck.this, "修改成功", Toast.LENGTH_SHORT).show();
			}else if(!two.equals(password01)){
					Toast.makeText(MyIDCheck.this, "旧密码输入错误", Toast.LENGTH_SHORT).show();
					temp();
			}else if(!four.equals(three)){
					Toast.makeText(MyIDCheck.this, "两次新密码输入不一致", Toast.LENGTH_SHORT).show();
					temp();
			}
		}
